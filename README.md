### Hikaru Oga

### About Me:

Language:
![](https://skillicons.dev/icons?i=html,css,js,bootstrap,vue,c,cs,dotnet,py)

Infrastructure:
![](https://skillicons.dev/icons?i=aws,azure,terraform,ansible,bash,docker,githubactions,jenkins,linux,mysql)

Communications:
![](https://skillicons.dev/icons?i=git,github,md,vscode,vim,)

<!--
**hikaru8079/hikaru8079** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
